...
class BlockStorageTraceStats:
    ...
    def _get_jump_distance(self, req):
        ...
        return start_offset - prev_end_offset


    def _get_iat(self, req):
        return req["ts"] - self._prev_req["ts"]
    ...
    def _track_seq_access(self, req):
        ...
        self._jump_distance_pstats.add_data(self._get_jump_distance(req))
    ...
    def _track_popularity(self, req):
        ...
        if req['op'] == 'r': 
            ...
            for page_index in range(req["start_page"], req["end_page"]+1):
                ...
                if self._scan_length > 1:
                    if req['op'] == 'r':
                        self._scan_read_count += 1 
                    else:
                        self._scan_write_count += 1 
    ...
    def get_stat(self):
        ...
        for percentile, percentile_val in zip(self._jump_distance_pstats.percentiles_tracked, self._jump_distance_pstats.get_percentiles()):
            stat['jd_p{}'.format(percentile)] = percentile_val
        stat['jd_avg'] = self._jump_distance_pstats.get_mean()
