...
class RunExperiment:
    ...
    def experiment_running(self, config, workload, cur_iteration):
       live_s3_key = get_s3_key("live", workload, config, cur_iteration)
       done_s3_key = get_s3_key("done", workload, config, cur_iteration)
    ...
    def run(self):
        for experiment_entry in self.experiment_list:
            ...
            kwargs = {}
            if "nvmCacheSizeMB" in experiment_entry["nvmCacheSizeMB"]:
                kwargs["nvmCacheSizeMB"] = experiment_entry["nvmCacheSizeMB"]
            ...
            for cur_iteration in range(self.iteration_count):
                ...
                live_s3_key_prefix = get_s3_key("live", workload, config, cur_iteration)
                ...
                done_s3_key_prefix = get_s3_key("live", workload, config, cur_iteration)
                self.s3.upload_s3_obj("{}/{}".format(done_s3_key_prefix, self.config_file_path.name), str(self.config_file_path.absolute()))
                self.s3.upload_s3_obj("{}/{}".format(done_s3_key_prefix, self.experiment_output_path.name), str(self.experiment_output_path.absolute()))
